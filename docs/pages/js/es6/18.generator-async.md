...
Promise 问题：代码冗余，通过 Promise包装后，不易理解（全是then）。
...
1. 传值调用（call by value）：即在进入函数体之前，就计算x + 5的值。在调用前求职，可能造成性能损失
...
run函数，就是一个 Generator 函数的自动执行器。通过判断 done 来确定是否终止。如果未终止，将 next函数（thunk函数）通过递归传递。
