...
Proxy用于修改默认行为。属于元编程（meta programming），即对编程语言进行编程。
...
第三个参数：指向原始读操作对象，一般为Proxy实例

getReceiver属性：获取receiver
...
1. 对象不可配置（`configurable`），或禁止扩展，has拦截将报错
...
1. 由于this指向的变化，导致 Proxy 无法代理目标对象
