...
export default defineConfig({
  ...
  themeConfig: {
    ...
    footer: {
      message:
        '备案号：<a href="https://beian.miit.gov.cn/" target="_blank">蜀ICP备2022016215号-1 	</a>',
      copyright: "",
    },
    algolia: {
      appId: "K106B03X2S",
...
function generateJson(test = false) {
  if (!test) {
    fs.writeFileSync(
      path.resolve(__dirname, "../public/sidebar.json"),
      JSON.stringify(sidebar, null, "\t")
    );
    fs.writeFileSync(
      path.resolve(__dirname, "../public/nav.json"),
      JSON.stringify(nav, null, "\t")
    );
    fs.writeFileSync(
      path.resolve(__dirname, "../public/pageInfo.json"),
      JSON.stringify(pages, null, "\t")
    );
  } else {
    fs.writeFileSync(
      path.resolve(__dirname, "../public/sidebar-test.json"),
      JSON.stringify(sidebar, null, "\t")
    );
  }
}
